WITH months AS (
    ...
    select
        ...
        case
            ...
            when repl_c.replacement_activated_effective_month is not null then (repl_c.replacement_activated_effective_month - interval '1 month')
        ...
        case
            ...
            when c.previous_contract_end_month > c.start_month then c.activated_effective_month
            -- Handle activated effective month following the contract duration
            when c.type_calculated = 'New Business' and c.activated_effective_month > c.end_month then c.start_month
            -- If New Business, Cancellation, or Expiration, use activated effective month
            when c.type_calculated in ('New Business', 'Cancellation', 'Expiration') then c.activated_effective_month
            -- Otherwise, use activated_effective_month
            else c.activated_effective_month
        ...
        c.contract_active,
        c.akamai_sales_rep,
        c.industry as primary_industry,
        c.sub_industry as primary_sub_industry,
    ...
    select
        ...
        -- NRR
        case
            when c.reporting_month >= c.contract_start_date and c.type_reporting in ('Event', 'High Traffic Event') then c.mrr_gross
            else 0
        end as nrr_gross
    ...
    select
        ...
        case
            when c.commit_type != 'Billion records per month' then coalesce(u.total_bytes, 0)
