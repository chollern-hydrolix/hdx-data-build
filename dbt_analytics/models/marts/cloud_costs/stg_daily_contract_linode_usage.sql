...
with deployments_by_contract as (
    ...
    select
        ...
        d.clean_cluster_hostname as cluster_hostname,
    ...
    -- Aggregated Linode usage by Cluster and Reporting Date
    select
        cluster_label,
        reporting_date,
        sum(hdx_amount) as total_amount
    from {{ref('stg_daily_linode_resource_billing')}}
    where cloud_account = 'prod'
    group by 1, 2
    ...
    select
        d.*,
        coalesce(l.reporting_date, e.date) as usage_date,
        case
    ...
    left join {{ref('stg_daily_shared_cluster_project_usage_estimate_pct')}} e
        on d.hdx_deployment_name = e.merge_name
        and d.contract_start_date <= e.date
        and d.contract_end_date >= e.date
    left join shared_instance_usage s
        on d.cluster_hostname = s.hostname
        and e.date = s.reporting_date
), deployments_grouped as (
    select
        contract_id,
        usage_date as reporting_date,
