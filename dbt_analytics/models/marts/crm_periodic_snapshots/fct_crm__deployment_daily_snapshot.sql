{{
    config(
        ...
        unique_key=['salesforce_id', 'snapshot_date'],
        incremental_strategy='delete+insert',
        indexes=[
            {'columns': ['snapshot_date']},
            {'columns': ['salesforce_id']},
            {'columns': ['salesforce_id', 'snapshot_date'], 'unique': True}
        ]
    )
}}
...
with new_snapshot_dates as (
    ...
    from {{ ref('snapshot_crm__deployment') }}
), joined as (
    select
        v.*,
        d.snapshot_date
    ...
    select distinct on (salesforce_id, snapshot_date)
        *
    from joined
    order by salesforce_id, snapshot_date, dbt_valid_from desc
)
select
    {{ dbt_utils.star(
        from=ref('snapshot_crm__deployment'),
